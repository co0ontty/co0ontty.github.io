...
使用bash反弹shell，但是由于`Runtime.getRuntime().exec()`中无法使用管道符等bash需要的方法，所以我们要通过对反弹shell的命令进行一次编码来执行。

编码后，使用Java反序列化工具`ysoserial.jar`生成poc.ser

![5d005f821f9a647202](https://i.loli.net/2019/06/12/5d005f821f9a647202.png)

将poc.ser作为POST Body发送至`/invoker/readonly`即可反弹shell
